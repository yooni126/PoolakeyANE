package com.doitflash.starling
{
	...
	public class MySprite extends Sprite 
	{
		...
		public function getBitmapData($fresh:Boolean=true):BitmapData
		{
			if (_bitmapData && !$fresh)
			{
				return _bitmapData;
			}
		...
		public function getTexture($fresh:Boolean=true):Texture
		{
			if (_texture && !$fresh)
			{
				return Texture.fromTexture(_texture);
			}
			
			if(_texture) _texture.dispose();
			_texture = Texture.fromBitmapData(getBitmapData());
