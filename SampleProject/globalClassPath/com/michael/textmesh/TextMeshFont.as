...
package com.michael.textmesh 
{
	...
	public class TextMeshFont implements ITextCompositor
	{
		...
		public var distanceFieldStyle:DistanceFieldStyle;
		...
		public function TextMeshFont(texture:Texture=null, fontXml:XML=null) 
		{
			...
			distanceFieldStyle = new DistanceFieldStyle();
                                      ...
                                      format:TextFormat, options:TextOptions=null):void
        {
            ...
            for (var i:int=0; i<numChars; ++i)
            {
				...
				if (charLocation.italic) _helperImage.skewX = deg2rad(10);
				else _helperImage.skewX = deg2rad(0);
				
				meshBatch.addMesh(_helperImage, null, charLocation.transparent);
                                      ...
                                      format:TextFormat, options:TextOptions):Vector.<CharLocation>
        {
                            ...
                            if (charLocation.x + char.width > containerWidth)
                            {
                                if (wordWrap)
                                {
									...
									for (j = 0; j < numCharsToRemove; ++j){ // faster than 'splice'
										...
										if (currentLine[currentLine.length - 1].tagLength > 0){
											hasTag = true;
										}
                                        currentLine.pop();
									}
			...
			if (isRTL){
				for (var ii:int = 0; ii < sLines.length; ii++){
					currentLine = sLines[ii].slice();
					currentX = 0;
					currentLine = currentLine.reverse();
					for (var jj:int = 0; jj < currentLine.length ; jj++){
						currentLine[jj].x = currentX;
						finalScale = 1
						if (currentLine[jj].char.isIcon){
							finalScale = getChar(84).height / currentLine[jj].char.height;
						}
						currentX += currentLine[jj].char.xAdvance * finalScale;
					}
					sLines[ii] = currentLine;
				}	
			}
